...
public class CompressString2 {
	...
	public int solution(String s) {
		...
		for (int i = 1; i <= length / 2; i++) {
			...
			while ((pointer + i) <= length) {
				if (!s.substring(pointer, pointer + i).equals(str)) {
					if (count > 1)
						temp += count + "";
					temp += str;
					str = s.substring(pointer, pointer + i);
					count = 1;
				} else {
					count++;
				}
			...
			// pointer 초과되어서 끝까지 안붙었을때
			if (pointer + i > length) {
				temp += s.substring(pointer);
			}
	...
	public static void main(String[] args) {
		...
		CompressString2 compressString2 = new CompressString2();
		compressString2.solution(s);
	}
}
